...
function Weather() {
  let weather_data = useSelector((state) => {
    return state.data;
  });
    ...
    <Fragment>
      ...
      {loading ? (
        <div className=" d-flex align-items-center justify-content-center">
          Waiting until entering city name in search
          field.........................{" "}
        </div>
      ) : (
        <div className={`${styles.weather_info}`}>
          ...
          {showDangerAlert ? (
            ...
            <Container className={`h-100 w-75 `}>
              ...
              <h2
                className={`text-center d-flex align-items-center justify-content-center h-25`}
              >
                {weather_data.name} {weather_data.sys.country}
              </h2>
              <div
                className={` ${styles.background} d-flex align-items-center justify-content-between h-75`}
                style={{
                  backgroundImage:
                    (weather_data.main.temp - 273.5).toFixed(2) > 37
                      ? `url(${sunny})`
                      : (weather_data.main.temp - 273.5).toFixed(2) < 37 &&
                        (weather_data.main.temp - 273.5).toFixed(2) > 20
                      ? `url(${warm})`
                      : (weather_data.main.temp - 273.5).toFixed(2) < 20 &&
                        (weather_data.main.temp - 273.5).toFixed(2) > 10
                      ? `url(${rain})`
                      : (weather_data.main.temp - 273.5).toFixed(2) < 10
                      ? `url(${snow})`
                      : `url(${windy})`,
              ...
              >
                ...
                <div className={`${styles.content} h-100 w-25`}>
                  <h2
                    className={`w-100 h-25 d-flex justify-content-center align-items-center `}
                  >
                    temp
                  </h2>
                  <div className={`h-50 w-100`}>
                    <h3 className={`text-center`}>
                      {weather_data.main.temp} K
                    </h3>
                    <h3 className={`text-center`}>
                      {(weather_data.main.temp * 1.8 - 459.67).toFixed(2)}{" "}
                      &#8457;
                    </h3>
                    <h3 className={`text-center`}>
                      {(weather_data.main.temp - 273.15).toFixed(2)} &#8451;
                    </h3>{" "}
                  </div>
                </div>
                {/*display humidity */}
